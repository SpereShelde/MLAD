...
df = pd.read_csv(os.path.join(ROOT_DIR, 'data', 'testbed', 'carpet.csv'))
df = df.drop(['idx'], axis=1)

#
# # df['Throttle_Control'][:1000].plot(color='red')
# df = df.drop(['Servo', 'Servo_Control', 'Voltage', 'Throttle_Control', 'Throttle_A', 'Throttle_B', 'Angular_Z', 'Linear_Y'], axis=1)
...
kf_vel_z = SimpleKF(0.44, 0.44, 0.4)
vel_z = df['Linear_Z']
vel_z_kf = []
for vz in vel_z:
...
kf_acc_x = SimpleKF(1.64, 1.64, 0.8)
...
kf_acc_y = SimpleKF(1.64, 1.64, 0.8)
...
kf_acc_z = SimpleKF(3.29, 3.29, 0.8)
...
kf_ang_x = SimpleKF(0.09, 0.09, 0.6)
ang_x = df['Angular_X']
ang_x_kf = []
for angx in ang_x:
...
kf_ang_y = SimpleKF(0.13, 0.13, 0.6)
ang_y = df['Angular_Y']
ang_y_kf = []
for angy in ang_y:
...
kf_ang_z = SimpleKF(0.07, 0.07, 0.4)
ang_z = df['Angular_Z']
ang_z_kf = []
for angz in ang_z:
